...
const renderSearchPayload = (payload = {}) => {
    ...
    if (users.length) {
        ...
        sections.push(sectionWrapper('Kullanicilar', html));
    }
    ...
    if (posts.length) {
        ...
        sections.push(sectionWrapper('Yazilar', html));
    }
        ...
        const html = categories.map((category) => resultRow(`
            <div class="site-search-row-stack">
                ...
                <div class="site-search-row-copy">
                    ...
                    <p class="site-search-row-meta">Kategori</p>
                </div>
            </div>
        `, category.url ?? '#')).join('');

        sections.push(sectionWrapper('Kategoriler', html));
    }
        ...
        const html = tags.map((tag) => resultRow(`
            <div class="site-search-row-stack">
                ...
                <div class="site-search-row-copy">
                    ...
                    <p class="site-search-row-meta">Etiket</p>
                </div>
            </div>
        `, tag.url ?? '#')).join('');

        sections.push(sectionWrapper('Etiketler', html));
    }
    ...
    if (pages.length) {
        ...
        sections.push(sectionWrapper('Sayfalar', html));
    }
...
const loadNotifications = async (force = false) => {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Bildirim istegi basarisiz oldu');
        }
...
const logoMenuRoot = document.querySelector('[data-logo-menu]');
const logoMenuBtn = logoMenuRoot?.querySelector('[data-logo-menu-btn]');
const logoMenuPanel = logoMenuRoot?.querySelector('[data-logo-menu-panel]');
const logoMenuClose = logoMenuRoot?.querySelector('[data-logo-menu-close]');

const closeLogoMenu = () => {
    if (!logoMenuPanel || !logoMenuBtn) return;
    logoMenuPanel.classList.add('hidden');
    logoMenuBtn.setAttribute('aria-expanded', 'false');
};

const toggleLogoMenu = () => {
    if (!logoMenuPanel || !logoMenuBtn) return;
    logoMenuPanel.classList.toggle('hidden');
    const isOpen = !logoMenuPanel.classList.contains('hidden');
    logoMenuBtn.setAttribute('aria-expanded', String(isOpen));
};

if (logoMenuBtn && logoMenuPanel) {
    ...
    logoMenuClose?.addEventListener('click', (event) => {
        event.preventDefault();
        closeLogoMenu();
    });
    ...
    document.addEventListener('click', (event) => {
        const target = event.target;
        if (target instanceof Element && logoMenuRoot?.contains(target)) return;
        closeLogoMenu();
    });
