...
<head>
    ...
    <title>@yield('title', __('Activity Log')) - {{ config('app.name') }}</title>
...
<body class="font-sans antialiased bg-gray-50 min-h-screen">
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                <div class="flex items-center space-x-6 overflow-x-auto">
                    ...
                    <a href="{{ route('messages.index') }}" class="text-sm text-gray-600 hover:text-gray-900 transition whitespace-nowrap">{{ __('Messages') }}</a>
                    <a href="{{ route('kyc.index') }}" class="text-sm text-gray-600 hover:text-gray-900 transition whitespace-nowrap">{{ __('KYC') }}</a>
                    <a href="{{ route('activity-log.my') }}" class="text-sm font-medium text-primary-600 hover:text-primary-700 transition whitespace-nowrap">{{ __('Activity') }}</a>
                    @auth
    ...
    @if (session('success'))
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-4">
            <div class="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-alma text-sm">{{ session('success') }}</div>
        </div>
    @endif
