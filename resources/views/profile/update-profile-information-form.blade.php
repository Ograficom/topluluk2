        ...
        <div class="settings-card__body space-y-6">
            ...
            <div class="relative">
                <div
                    ...
                    class="group relative h-36 w-full overflow-hidden rounded-2xl bg-slate-100 dark:bg-slate-800 sm:h-44"
                >
                    ...
                    @else
                        <div x-show="!coverPreview" class="flex h-full w-full items-center justify-center text-sm text-slate-400 dark:text-slate-500">
                            Kapak görseli yok
                        </div>
                    @endif
                ...
                >
                    ...
                    <div class="relative h-20 w-20 overflow-hidden rounded-full border-4 border-white dark:border-slate-900 bg-slate-100 dark:bg-slate-800 shadow-md sm:h-[5.5rem] sm:w-[5.5rem]">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 pt-11 sm:pt-12">
                <div class="flex flex-wrap gap-x-4 gap-y-1 text-xs">
                    @if ($this->user->profile_photo_path)
                        <button type="button" class="font-medium text-slate-500 dark:text-slate-400 transition hover:text-red-600" wire:click="deleteProfilePhoto">
            ...
            <div>
                ...
                @if (Laravel\Fortify\Features::enabled(Laravel\Fortify\Features::emailVerification()) && ! $this->user->hasVerifiedEmail())
                    <div class="mt-2 flex flex-wrap items-center gap-2 text-sm">
                        <span class="inline-flex items-center gap-1 rounded-full bg-amber-50 dark:bg-amber-500/10 px-2.5 py-1 text-xs font-medium text-amber-700 dark:text-amber-400">
            ...
            <div>
                <div class="mb-1.5 flex items-center justify-between">
                    ...
                    <span class="text-xs text-slate-400 dark:text-slate-500" data-bio-count>{{ mb_strlen((string) ($this->state['bio'] ?? '')) }}/2000</span>
                </div>
            ...
            {{-- Social links --}}
            <div>
                <h3 class="mb-3 text-sm font-semibold text-slate-900 dark:text-slate-100">Sosyal bağlantılar</h3>
            ...
            <div class="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/70 p-4">
                <div class="flex items-start justify-between gap-4">
                    <label for="is_verified" class="flex items-start gap-3">
                        <span class="mt-0.5 inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-500/15 text-blue-600 dark:text-blue-400">
                            <iconify-icon icon="lucide:badge-check" style="font-size: 16px;"></iconify-icon>
                        </span>
                        <span>
                            <span class="block text-sm font-medium text-slate-900 dark:text-slate-100">Mavi tik / onaylı hesap</span>
                            <span class="mt-0.5 block text-xs text-slate-500 dark:text-slate-400">Profilinde doğrulama rozeti göster.</span>
                        </span>
                    </label>
                ...
                <div class="mt-4 grid grid-cols-1 gap-5 border-t border-slate-200/80 dark:border-slate-700/80 pt-4 sm:grid-cols-2">
                    <div>
                ...
                <p class="mt-3 text-xs leading-relaxed text-slate-500 dark:text-slate-400">SVG kodunu yapıştırabilirsin. Boş bırakılırsa varsayılan kullanılır.</p>
                <x-input-error for="verification_badge_svg" class="mt-2" />
            </div>
        </div>

        <div class="flex items-center justify-end gap-3 border-t border-slate-100 dark:border-slate-800 px-5 py-4 sm:px-6">
