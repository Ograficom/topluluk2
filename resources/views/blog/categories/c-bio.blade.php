...
<section class="rounded-xl p-5" data-category-hero>
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div class="space-y-2">
            <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Kategoriler</p>
            <h1 class="text-2xl font-semibold text-slate-900 dark:text-slate-100">Topluluk atlasi</h1>
            ...
            <div class="flex flex-wrap gap-2 text-xs font-semibold text-slate-700 dark:text-slate-300">
                ...
                <span class="inline-flex items-center gap-2 rounded-full bg-slate-100 dark:bg-slate-800 px-3 py-1">
                    Bu sayfada <span data-category-visible-count class="text-slate-900 dark:text-slate-100">{{ $listedCategories }}</span>
                </span>
                <span class="inline-flex items-center gap-2 rounded-full bg-slate-100 dark:bg-slate-800 px-3 py-1">
        ...
        <div class="w-full max-w-sm rounded-xl p-4">
            <div class="flex items-center gap-3">
                <div class="relative h-12 w-12 overflow-hidden rounded-xl bg-slate-100 dark:bg-slate-800 ring-1 ring-slate-200 dark:ring-slate-700">
                    @if ($avatar)
                        <img src="{{ $avatar }}" alt="{{ $userName }}" class="h-full w-full object-cover">
                    @else
                        <div class="flex h-full w-full items-center justify-center text-sm font-semibold text-slate-700 dark:text-slate-300">
                            {{ $initials }}
                        </div>
                    @endif
                </div>
                <div class="min-w-0">
                    ...
                    @if ($userUsername)
                        <p class="text-xs text-slate-500 dark:text-slate-400 truncate">{{ '@' . $userUsername }}</p>
                    @endif
                </div>
            </div>
            <div class="mt-3 grid grid-cols-3 gap-2 text-center text-[11px] font-semibold text-slate-600 dark:text-slate-400">
                <div class="rounded-lg bg-slate-50 dark:bg-slate-800 px-3 py-2">
                    ...
                    <p class="text-sm font-bold text-slate-900 dark:text-slate-100">{{ number_format($postsCount) }}</p>
                </div>
                <div class="rounded-lg bg-slate-50 dark:bg-slate-800 px-3 py-2">
                    <p class="text-[10px] uppercase tracking-wide text-slate-400">Takipci</p>
                    <p class="text-sm font-bold text-slate-900 dark:text-slate-100">{{ number_format($followersCount) }}</p>
                </div>
                <div class="rounded-lg bg-slate-50 dark:bg-slate-800 px-3 py-2">
                    <p class="text-[10px] uppercase tracking-wide text-slate-400">Takip</p>
                    <p class="text-sm font-bold text-slate-900 dark:text-slate-100">{{ number_format($followingsCount) }}</p>
                </div>
            </div>
        </div>
    </div>

    <div class="mt-4 flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-700 dark:text-slate-300" data-category-filters>
            @foreach ([
                ['key' => 'all', 'label' => 'Hepsi'],
