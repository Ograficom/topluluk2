...
<x-app-layout>
    <div class="messages-page px-3 py-4 sm:px-4 sm:py-8">
        ...
        <div class="site-main-shell">
            ...
            @if ($errors->any())
                <div class="mb-4 rounded-2xl border border-rose-200 dark:border-rose-800 bg-rose-50 dark:bg-rose-950 px-4 py-3 text-sm text-rose-700 dark:text-rose-300">
                    {{ $errors->first() }}
                </div>
            @endif
            ...
            <div class="mb-4 flex flex-col gap-3 sm:mb-6 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white sm:text-3xl">{{ __('messages.title') }}</h1>
                </div>
            ...
            <div class="messages-shell {{ count($threads) === 0 ? 'messages-shell--simple' : '' }}">
                ...
                @if (count($threads) > 0)
                    <section class="messages-card messages-main-panel hidden lg:flex">
                        <div class="border-b border-gray-200 dark:border-slate-700 p-4 flex items-center gap-3">
                            <div class="flex h-11 w-11 items-center justify-center rounded-full bg-gray-200 dark:bg-slate-700 text-sm font-semibold text-gray-700 dark:text-slate-300 shrink-0">
                                --
                            </div>
                            <div class="min-w-0">
                                <h2 class="truncate text-base font-semibold text-gray-900 dark:text-white sm:text-lg">{{ __('messages.select_conversation') }}</h2>
                                <p class="text-sm text-gray-500 dark:text-slate-400">{{ __('messages.select_conversation_hint') }}</p>
                            </div>
                        </div>

                        <div class="flex-1 bg-gray-50 dark:bg-slate-900 p-3 sm:p-4">
                            <div class="messages-empty-state flex h-full min-h-[360px] flex-col items-center justify-center px-6 py-10 text-center">
                                <div class="flex h-14 w-14 items-center justify-center rounded-full bg-gray-200 dark:bg-slate-700 text-gray-500 dark:text-slate-400">
                                ...
                                <h3 class="mt-4 text-lg font-semibold text-gray-900 dark:text-white sm:text-xl">{{ __('messages.ready_title') }}</h3>
                                <p class="mt-2 max-w-md text-sm leading-6 text-gray-500 dark:text-slate-400">
