@forelse ($messages as $message)
    ...
    <div class="message-fade flex {{ $isMine ? 'justify-end' : 'justify-start' }}">
        <div class="flex max-w-full flex-col {{ $isMine ? 'items-end' : 'items-start' }}">
            <div class="messages-bubble {{ $isMine ? 'messages-bubble--mine' : 'messages-bubble--other' }}">
                @if ($attachmentUrl)
                    ...
                    @elseif (str_starts_with($attachmentMime, 'audio/'))
                        <div class="mb-3 rounded-xl {{ $isMine ? 'bg-white/10' : 'bg-gray-50 dark:bg-slate-800' }} p-3">
                    ...
                    @else
                        <a
                            ...
                            target="_blank"
                            rel="noopener"
                            class="mb-3 flex items-center gap-3 rounded-xl {{ $isMine ? 'bg-white/10 text-white' : 'border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 text-gray-700 dark:text-slate-300' }} px-4 py-3"
                        >
                            <span class="inline-flex h-10 w-10 items-center justify-center rounded-full {{ $isMine ? 'bg-white/10' : 'bg-white dark:bg-slate-700' }}">
            ...
            <div class="mt-1 px-1 text-[11px] text-gray-400 dark:text-slate-500">
...
@empty
    <div class="messages-empty-state flex min-h-[280px] flex-col items-center justify-center px-6 py-10 text-center">
        <div class="flex h-14 w-14 items-center justify-center rounded-full bg-gray-200 dark:bg-slate-700 text-gray-500 dark:text-slate-300">
        ...
        <p class="mt-4 text-base font-medium text-gray-700 dark:text-slate-300">{{ __('messages.thread.empty_title') }}</p>
        <p class="mt-1 max-w-md text-sm text-gray-500 dark:text-slate-400">{{ __('messages.thread.empty_description') }}</p>
    </div>
@endforelse
