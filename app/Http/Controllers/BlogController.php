...
class BlogController extends Controller
{
    ...
    public function archive(Request $request)
    {
        ...
        $from = $this->parseArchiveDate($request->query('from'));
        $to = $this->parseArchiveDate($request->query('to'));
        ...
        $postsQuery = Post::query()
            ...
            ->withCount(['comments', 'reactions'])
            ->when($from, fn (Builder $query) => $query->where('published_at', '>=', $from->startOfDay()))
            ->when($to, fn (Builder $query) => $query->where('published_at', '<=', $to->copy()->endOfDay()))
            ->orderByDesc('published_at');
        ...
        return view('blog.filtre.archive', [
            'posts' => $posts,
            'reactionTypes' => $reactionTypes,
            'archiveFrom' => $from,
            'archiveTo' => $to,
    ...
    private function parseArchiveDate(mixed $value): ?Carbon
    {
