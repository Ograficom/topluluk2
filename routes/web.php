...
Route::post('verify', function () {
    return response()->json(['response' => ['code' => 200, 'message' => 'Verified']]);
})->name('verify');
...
/*
|--------------------------------------------------------------------------
| Pro Features: Messages, KYC, Activity Logs
|--------------------------------------------------------------------------
*/
...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::name('messages.')->prefix('messages')->group(function () {
        ...
        Route::post('/{conversation}/reply', [MessagesController::class, 'reply'])->name('reply');
        Route::delete('/{conversation}', [MessagesController::class, 'destroy'])->name('destroy');
    });
    ...
    Route::name('kyc.')->prefix('kyc')->group(function () {
        ...
        Route::get('/resubmit', [KycController::class, 'resubmit'])->name('resubmit');
    });

    // Activity Logs
    Route::name('activity-log.')->prefix('activity-log')->group(function () {
        Route::get('/my', [ActivityLogController::class, 'myLogs'])->name('my');
    });

    // Admin: KYC management
    Route::middleware(['role:administrator'])->group(function () {
        Route::prefix('kyc/admin')->name('kyc.')->group(function () {
            Route::get('/', [KycController::class, 'admin'])->name('admin');
            Route::post('/{kycDocument}/approve', [KycController::class, 'approve'])->name('approve');
            Route::post('/{kycDocument}/reject', [KycController::class, 'reject'])->name('reject');
        });

        // Admin: Activity logs
        Route::prefix('activity-log/admin')->name('activity-log.')->group(function () {
            Route::get('/', [ActivityLogController::class, 'index'])->name('index');
            Route::get('/user/{user:username}', [ActivityLogController::class, 'user'])->name('user');
        });
    });
});
